...
async def setup_hook():
    ...
    # Планирование уведомлений о собеседованиях
    for slot in bot.interviews:
        ...
        bot.loop.create_task(notify(slot))

# ---------- КОМАНДЫ ГОС. ВОЛНЫ ----------
@bot.command()
async def sobes(ctx, *, text: str):
    """Запись на гос. волну: !sobes Семья | 13:00 | обзвон"""
    ...
    delay = (slot_dt - datetime.now()).total_seconds()
    async def notify(slot):
        await asyncio.sleep(delay)
        if slot in bot.interviews:
            ...
            if ch:
                if slot.get("type") == "обзвон":
                    vc = f"<#{VOICE_CHANNEL_ID}>" if VOICE_CHANNEL_ID else "голосовой канал"
                    txt = f"📢 <@{slot['user_id']}>, просьба зайти в голосовой канал {vc}"
                else:
                    txt = f"⏰ <@{slot['user_id']}>, начинается собеседование, заходите"
                await ch.send(txt)
    bot.loop.create_task(notify(slot))
    type_text = "обзвон" if type_str == "обзвон" else "собеседование"
    await ctx.reply(f"✅ Семья **{family}** записана на **{type_text}** в **{time_str}**.", mention_author=False)
...
async def cancel(ctx, *, text: str = None):
    """Отмена записи: !cancel Семья | 13:00"""
    if not text:
